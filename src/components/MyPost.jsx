...
function MyPost() {
  ...
  <div className='m-2 w-90 md:w-[90%] shadow-2xl h-auto relative bg-white rounded-xl cursor-pointer' key={post._id}>
    ...
    <div className='flex justify-between'>
       {/* Status Tag */}
        <div className="m-5 mt-7 md:p-2">
          {post.approved === null && (
            <span className="px-3 py-2 bg-yellow-600 text-white text-md rounded-full">Pending</span>
          )}
          {post.approved === true && (
            <span className="px-3 py-2 bg-green-700 text-white text-sm rounded-full">Approved</span>
          )}
          {post.approved === false && (
            <span className="px-3 py-2 bg-red-700 text-white text-sm rounded-full">Rejected</span>
          )}
        </div>
    ...
    <div className='md:m-2 md:p-4 max-w-260' onClick={() => handleNavigate(post._id)}>
      ...
      <div className="mx-7 md:mx-0 md:m-3 md:p-3">
        ...
        <div className="my-2 md:p-1 md:m-2">
          <span className='p-1 px-3 mr-2 bg-green-700 text-white w-23 h-10 text-center rounded-2xl'>Created at: </span>
